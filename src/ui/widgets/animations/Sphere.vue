...
const hexToHsl = (c: string, T: typeof import('three')) => {
  const col = new T.Color(c);
  const hsl = { h: 0, s: 0, l: 0 };
  (col as any).getHSL(hsl);
  return hsl;
};
const hslToColor = (h: number, s: number, l: number, T: typeof import('three')) => {
  const c = new T.Color();
  c.setHSL(h, s, l);
  return c;
};
...
const maybeRunMotionLoop = () => {
  ...
  const step = () => {
    ...
    const moving =
      props.autoRotate || Math.abs(angVelX) > 0.01 || Math.abs(angVelY) > 0.01 || dragging;
...
const init = async () => {
  ...
  const targetDetail = Math.max(
    0,
    Math.min(2, el.clientWidth < 560 ? props.detail - 1 : props.detail)
  );
  const geom = new THREE.IcosahedronGeometry(props.radius, targetDetail);
