...
const setCellAlive = (x: number, y: number, vitality = 6) => {
  ...
  const i = yy * w + xx;
  G[i] = 1;
  A[i] = Math.min(255, (A[i] ?? 0) + vitality);
};
const brushRCells = 2,
  brushDensity = 0.85,
  brushVitality = 6;
...
const onPointerMovePaint = (e: PointerEvent) => {
  ...
  if (now - lastPaint < paintThrottleMs) return;
  lastPaint = now;
  const host = wrapRef.value;
  if (!host) return;
  const rect = host.getBoundingClientRect();
